...
class AutodetectTokensProvider: AutodetectTokensProviderType {
    ...
    private func autoDetectTransactedTokensImpl(wallet: AlphaWallet.Address, server: RPCServer, erc20: Bool) -> Promise<Void> {
        return Promise<Void> { seal in
            ...
            GetContractInteractions(queue: self.queue).getContractList(address: wallet, server: server, startBlock: startBlock, erc20: erc20) { [weak self, weak storage, weak tokenProvider] contracts, maxBlockNumber in
                guard let strongSelf = self, let storage = storage, let tokenProvider = tokenProvider else { return }
                defer {
                    seal.fulfill(())
                }
                ...
                for eachContract in contractsToAdd {
                    tokenProvider.addToken(for: eachContract, server: server) { _ in
                        contractsPulled += 1
